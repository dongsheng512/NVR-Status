...
3. **新目录落位：** 新 GUI 放 `ui/`（或 `gui_qt/`），旧 `gui_app.py` 在切换默认入口前可保留对照。  
...
- [ ] 主窗布局：左配置 / 右结果 / 底状态栏  
- [ ] 配置档案：新建、另存、重命名、删除、导入、导出、切换  
- [ ] 设备：列表、添加/编辑对话框、删除、保存到档案  
- [ ] 扫描目标设备选择  
- [ ] 扫描设置（可折叠或分页）：回溯、workers、深度参数、保存路径等  
- [ ] 快速巡检 / 深度巡检；深度可选保存片段  
- [ ] 无 ffmpeg 时提示并可降级  
- [ ] 后台巡检不卡 UI；进度条 + 阶段文案  
- [ ] 结果：汇总、指标卡、预警、通道表、日志  
- [ ] 导出 CSV（utf-8-sig）/ TXT  
- [ ] 打开样片目录  
- [ ] 用户数据路径与 v1 兼容（同一 `profiles.json`）  

### 4.2 建议随重写完成（P1）

- [ ] 通道表排序、筛选（仅异常 / 仅离线）  
- [ ] 通道详情（可同一 model，不必第二套数据）  
- [ ] 日志分级着色 + 行数上限 + 复制  
- [ ] 浅色 / 深色 / 跟随系统  
- [ ] 窗口与 Splitter 比例 `QSettings` 记忆  
- [ ] 崩溃/异常写入 `app_data_dir()/logs/`  
- [ ] 巡检取消（cancel flag + UI 按钮）  
...
│   │   └── status_bar.py
│   └── resources/           # 可选 QSS
├── run_gui.py               # 改为启动 ui.app
├── gui_app.py               # 过渡期保留，默认入口切换后可归档
├── NVRStatus.spec           # 重写（PySide6）
└── build/                   # 更新脚本
```
...
| 4.7 | （可选）归档或删除 `gui_app.py` CTk 实现 |
...
| ID | 工作项 | 阶段 | 状态 |
|----|--------|------|------|
| W0 | 添加 PySide6 依赖与 `ui/` 骨架 | 0 | 待办 |
| W1 | ScanWorker + Signals | 1 | 待办 |
| W2 | MainWindow POC + 真机 | 1 | 待办 |
| W3 | 档案与设备 UI | 2 | 待办 |
| W4 | 扫描设置与双模式巡检 | 2 | 待办 |
| W5 | 结果区与导出 | 2 | 待办 |
| W6 | Model/View 表与主题 | 3 | 待办 |
| W7 | 打包 spec 与双平台 | 4 | 待办 |
| W8 | 文档/入口切换/发版 | 4 | 待办 |
